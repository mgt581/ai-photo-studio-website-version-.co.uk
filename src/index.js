...
function googlePlayPlanForItem(env, itemId, requestedPlan) {
  const productPlans = {
    [env.GOOGLE_PLAY_MONTHLY_SKU || 'aips_monthly_pro']: 'month',
    [env.GOOGLE_PLAY_YEARLY_SKU || 'aips_yearly_pro']: 'year'
  };
  const plan = productPlans[itemId] || String(requestedPlan || '').toLowerCase();
...
async function handleGooglePlayPurchase(request, env) {
  ...
  const plan = googlePlayPlanForItem(env, itemId, body.plan);

  if (!uid) return jsonResponse(request, { ok: false, error: 'uid-required' }, 400);
  if (!itemId) return jsonResponse(request, { ok: false, error: 'google-play-item-required' }, 400);
  if (!plan) return jsonResponse(request, { ok: false, error: 'invalid-google-play-product' }, 400);
  if (!purchaseToken) return jsonResponse(request, { ok: false, error: 'google-play-token-required' }, 400);

  return jsonResponse(request, {
    ok: false,
    error: 'google-play-verification-required'
  }, 501);
}
...
function handleRemoveObjectStatus(request, env) {
  return jsonResponse(request, {
    ok: true,
    providers: {
    ...
    bindings: {
      premiumStatus: Boolean(premiumStore(env))
    },
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === '/api/remove-object/status') {
      return handleRemoveObjectStatus(request, env);
    }
    ...
    if (url.pathname === '/google-play-purchase') {
      return handleGooglePlayPurchase(request, env);
    }
